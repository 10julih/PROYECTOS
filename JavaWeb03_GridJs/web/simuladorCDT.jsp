...
<h2 class="mt-5 text-center">CDT</h2>
<div class="container mt-5">
<div class="row">
<div class="col-sm">
...
if (monto >= 100000 && monto <= 25000000) {
double totalRendimientos = 0;
double inversionTotal = monto;

StringBuilder jsonBuilder = new StringBuilder("[");
for (int i = 1; i <= plazo; i++) {
double interesMensual = inversionTotal * (tasa) / (360 / (plazo));
totalRendimientos = interesMensual;
inversionTotal = monto;

jsonBuilder.append("[")
        .append("\"").append(nf.format(inversionTotal)).append("\",")
        .append("\"").append(nf.format(totalRendimientos)).append("\"],");
}
jsonBuilder.setLength(jsonBuilder.length() - 1); // Remove the last comma
jsonBuilder.append("]");
jsonData = jsonBuilder.toString();
} else {
out.println("No cumple con las condiciones del CDT");
}
...
columns: ["Inversion Inicial", "Rendimientos totales de la inversión"],
data: <%= jsonData%>,
search: true,
pagination: {
enabled: true,
...
widths: ['auto', 'auto'],
body: [
['Inversion Inicial', 'Rendimientos totales de la inversión'],
